...
def create_fake_hashes(info):
    ...
    return [b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'] * npieces
...
def create_tree(height):
    ...
    tree = [b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'] * treesize
    return tree
...
def fill_tree(tree, height, npieces, hashes):
    ...
    for level in range(height, 0, -1):
        for offset in range(int(pow(2, level) - 1), int(pow(2, level + 1) - 2), 2):
            ...
            digester.update(tree[offset])
            digester.update(tree[offset+1])
...
def get_piece_hashes(tree, height, npieces):
    startoffset = int(pow(2, height) - 1)
    hashes = [b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'] * npieces
    for offset in range(startoffset, startoffset + npieces):
